...
On top of these files, what is needed to make it **a production-ready application**, and why is a package the perfect infrastructure?
...
In this example, `library(purrr)` and `library(jsonlite)` will lead to a NAMESPACE conflict on the `flatten()` function and that will cause you some debugging headache (trust us, we have been there before).
...
-   `Vignettes` are longer form documentation that explain in more depth how to use your app. These are also useful if you need to detail the core functions of the application using a static document, notably for prototyping and/or for exchanging with the client. We will get back to `Vignettes` in Chapter \@ref(building-ispum-app) when we will talk about prototyping.
-   Function documentation. Every function in your package should come with its own documentation, even if it is just for your future self. "Exported" functions, the ones which are available once you run `library(myapp)`, should be fully documented and listed in the package help page. Internal functions need less documentation, but documenting them is the best way to be sure you can come back to the app in a few months and still know why things are the way they are, what the pieces of the apps are used for, and how to use these functions [^structure-1] .
-   If needed, you can build a `{pkgdown}` website, that can either be deployed on the web or kept internally. It can contain installation steps for I.T., internal features for use by developers, a user guide, etc.

[^structure-1]: `{roxygen2}` comes with a `@noRd` tag, that prevents the documentation from being built.
    This allows one to still write documentation using the same tags as the exported function, without the internal functions being documented in the end package.
...
Understanding all of this is not the purpose of this chapter: just remember that `{shiny}` input and output ids **have** to be unique, just like any id on a webpage, so that the browser knows where to put what it receives from R, and R knows what to listen to from the browser.
...
**`{shiny}` modules aim at three things: simplifying "id" namespacing, splitting the code base into a series of functions, and allowing UI/Server parts of your app to be reused. Most of the time, modules are used to doing the first two. In our case, we could say that 90% of the modules we write are never reused [^structure-2] ; they are here to allow us to split the code base into smaller, more manageable pieces**.
...
A production app almost always starts as a small Proof of Concept.
...
So, you will be better off starting on a solid foundation from the very beginning.
...
Each call to a module with `choice_server()` requires a different `id` argument that will allow creating various internal namespaces, preventing id conflicts [^structure-3] .
...
As you can see, the **app\_ui** contains a series of calls to `mod_ui(unique_id, ...)` function, allowing additional arguments like any other function:
...
# Module 1, which will allow one to select a number
choice_ui <- function(id) {
  ns <- NS(id)
  tagList(
...
It might also create some reactivity issues, as a lot of `reactive` function calls is harder to control and can lead to too much computation from the server.
...
R6 objects, created using the package of the same name, are "traditionnal" object-oriented programming implementations in R.
...
As this R6 object is not a reactive object and is not meant to be used as such, this reducing uncontrolled reactivity of the application, thus reduces the complexity of handling chain reactions across modules.
...
`{tidymodules}` [@R-tidymodules] is a package that helps to build shiny modules using an object-oriented paradigm, based on `{R6}`.
It automatically takes care of namespaces, and makes sharing data across modules easier.
...
-   **Business logic are the components with the core algorithms and functions that make your application specific to your area of work**.
...
If you keep everything together in the same file, you will end up having to rerun the app from scratch and spend five minutes clicking everywhere just to be sure you have correctly set the color palette for the graph on the last `tabPanel()`.
...
And of course, in the long run, even developers with knowledge of the structure can get lost, even more so when returning to a project that they have not worked on for a while.
